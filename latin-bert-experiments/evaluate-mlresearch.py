...
def evaluate(wp_tokenizer, text_before_lacuna, text_after_lacuna, model, truth):
    # tokenizer -> don't have to think about
    # lacuna -> this is the word thats trying to be predicted
    # text before/text after is the rest of the sentence
    # model doesnt need to be changed

    #to make this work with word bank, would have to add word bank parameter to the end
	...
	with torch.no_grad():
		...
		preds=preds[0]
		sortedVals=torch.argsort(preds[0][mask_id], descending=True)
		# probablility of all hte words predicited 

        # can have a line here that shrinks sorted vals to just the sorted values for those indexes 

        # before trying to filter, step one is doing the evalue where it considers all possible owrds
        
        # sorted values -> big nidex of values
		for k, p in enumerate(sortedVals[:50]):
            ...
            # reverse vocab ges actual words
            # reverse vocab with the new parameter
			...
			truth=truth.lower()
			# where it is checking to see if the accuracy, we have to remove all of the words that aren't in the word bank 
			if k == 0:
